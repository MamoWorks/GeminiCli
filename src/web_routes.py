...
class CredFileBatchActionRequest(BaseModel):
    ...
    model: Optional[str] = "gemini-2.5-pro"  # 测试模型，默认gemini-2.5-pro
...
async def serve_control_panel(request: Request):
    ...
    try:
        html_file_path = "client/dashboard.html"
...
@router.post("/creds/batch-test-alive")
async def batch_test_credentials_alive(request: Request, token: str = Depends(verify_token)):
    """
    批量测试所有凭证的有效性

    测试方法：发送实际的非流式API请求
    - HTTP 200 且有有效内容：账号可用，自动启用
    - HTTP 429 或其他非200状态码：账号不可用，自动禁用
    - HTTP 200 但内容为空：账号不可用，自动禁用
    - 请求异常/网络错误：保持当前状态
    """
    try:
        await ensure_credential_manager_initialized()
        ...
        try:
            ...
            test_model = body.get('model', 'gemini-2.5-pro')
        except:
        ...
        log.info(f"=" * 60)
        log.info(f"[批量测活] 开始批量测试 {len(all_credentials)} 个凭证，使用模型: {test_model}")
        log.info(f"=" * 60)
        ...
        # 并发测试所有凭证（限制并发数避免压力过大）
        async def test_single_credential(filename: str):
            """
            测试单个凭证的有效性
            通过发送实际API请求来验证账号是否可用
            """
            nonlocal valid_count, invalid_count, error_count

            log.info(f"[测活] 开始测试凭证: {filename}")
            ...
            try:
                ...
                # 检查项目ID
                if "project_id" not in credential_data:
                    error_count += 1
                    return {
                        "filename": filename,
                        "status": "error",
                ...
                if 'type' not in credential_data and all(key in credential_data for key in ['client_id', 'refresh_token']):
                    ...
                    log.debug(f"[测活] {filename} - 自动添加type字段")
                ...
                if not credential_data.get("access_token") and not credential_data.get("token"):
                    log.info(f"[测活] {filename} - 没有access_token，需要刷新")
                    should_refresh = True
                else:
                        ...
                        try:
                            if isinstance(expiry_str, str):
                                ...
                                if time_left <= 300:  # 5分钟缓冲
                                    log.info(f"[测活] {filename} - Token剩余时间: {int(time_left/60)}分钟，即将过期，需要刷新")
                                    should_refresh = True
                                else:
                                    log.info(f"[测活] {filename} - Token剩余时间: {int(time_left/60)}分钟，仍然有效")
                                    should_refresh = False
                        except Exception as e:
                            log.warning(f"[测活] {filename} - 解析过期时间失败: {e}，需要刷新")
                            should_refresh = True
                    else:
                        log.info(f"[测活] {filename} - 没有过期时间，需要刷新")
                        should_refresh = True

                # 如果需要刷新token
                if should_refresh:
                    if "refresh_token" not in credential_data or not credential_data["refresh_token"]:
                        log.warning(f"[测活] {filename} - Token已过期但没有refresh_token，无法刷新")
                        error_count += 1
                        return {
                            "filename": filename,
                            "status": "error",
                            "message": "Token已过期且无refresh_token",
                            "action": "无"
                        }

                    try:
                        log.info(f"[测活] {filename} - 开始刷新Token...")
                        ...
                        log.info(f"[测活] {filename} - Token刷新成功")
                    except Exception as e:
                        log.error(f"[测活] {filename} - Token刷新失败: {e}")
                        invalid_count += 1
                        # 禁用账号并记录错误码400
                        await credential_manager.set_cred_disabled(filename, True)
                        await credential_manager.record_api_call_result(filename, False, 400)
                        return {
                            "filename": filename,
                            "status": "invalid",
                            "message": f"Token刷新失败（无效凭证）: {str(e)}",
                            "action": "已禁用"
                        }

                # 最终检查是否有访问令牌
                if "token" not in credential_data and "access_token" not in credential_data:
                    error_count += 1
                    return {
                        "filename": filename,
                        "status": "error",
                ...
                log.debug(f"测试凭证: {filename}，使用模型: {test_model}")

                # 使用项目中已有的send_gemini_request函数发送请求
                from .google_chat_api import send_gemini_request

                # 创建一个临时的凭证管理器实例，只包含当前凭证
                class SingleCredentialManager:
                    """临时凭证管理器，只返回指定的单个凭证"""
                    def __init__(self, filename, credential_data):
                        self.filename = filename
                        self.credential_data = credential_data

                    async def get_valid_credential(self):
                        return (self.filename, self.credential_data)

                    async def record_api_call_result(self, filename, success, error_code=None):
                        pass  # 测试阶段不记录，避免重复记录

                temp_manager = SingleCredentialManager(filename, credential_data)

                try:
                    # 发送非流式请求
                    response = await send_gemini_request(test_payload, is_streaming=False, credential_manager=temp_manager)
                    ...
                    try:
                        ...
                        log.info(f"[测活] {filename} - HTTP {status_code}")
                        log.info(f"[测活] {filename} - 完整响应体: {json.dumps(response_data, ensure_ascii=False, indent=2)}")
                    except:
                        log.info(f"[测活] {filename} - HTTP {status_code}, 无法解析响应体")
                    ...
                    if status_code == 200:
                        ...
                        await credential_manager.set_cred_disabled(filename, False)
                        # 清除错误码
                        await credential_manager.record_api_call_result(filename, True, None)
                        log.info(f"[测活] {filename} - ✓ 账号有效（HTTP 200），已启用并清除错误码")
                        return {
                            "filename": filename,
                            "status": "valid",
                            "message": "账号有效，HTTP 200",
                            "action": "已启用"
                        }
                    else:
                        # 非200状态码 = 账号不可用
                        invalid_count += 1
                        await credential_manager.set_cred_disabled(filename, True)
                        await credential_manager.record_api_call_result(filename, False, status_code)

                        # 尝试获取错误详情
                        try:
                            error_detail = response.body.decode('utf-8')[:200]
                            log.warning(f"[测活] {filename} - ✗ HTTP {status_code}，错误详情: {error_detail}")
                        except:
                            error_detail = ""
                            log.warning(f"[测活] {filename} - ✗ HTTP {status_code}，无法获取错误详情")

                        return {
                            "filename": filename,
                            "status": "invalid",
                            "message": f"账号不可用，HTTP {status_code}: {error_detail}",
                            "action": "已禁用"
                        }
                    ...
                    # 请求失败
                    error_count += 1
                    log.error(f"[测活] {filename} - ✗ 请求失败: {req_error}")
                    return {
                        "filename": filename,
                        "status": "error",
                        "message": f"请求失败: {str(req_error)}",
                        "action": "保持当前状态"
                    }

            except Exception as e:
                error_count += 1
                log.error(f"[测活] {filename} - ✗ 测试异常: {e}")
                return {
                    "filename": filename,
                    "status": "error",
        ...
        for i in range(0, len(all_credentials), batch_size):
            ...
            batch_results = await asyncio.gather(*[test_single_credential(filename) for filename in batch])
            results.extend(batch_results)

            # 记录进度
            log.info(f"[批量测活] 进度: {min(i + batch_size, len(all_credentials))}/{len(all_credentials)}")

        # 构建响应
        message = f"批量测活完成：有效 {valid_count} 个，无效 {invalid_count} 个，错误 {error_count} 个"

        log.info(f"=" * 60)
        log.info(f"[批量测活] {message}")
        log.info(f"=" * 60)
        ...
        log.error(f"批量测活失败: {e}")
        raise HTTPException(status_code=500, detail=str(e))


@router.post("/creds/refresh-token/{filename}")
async def refresh_single_token(filename: str, token: str = Depends(verify_token)):
    """
    刷新单个凭证的access token
    ...
    try:
        ...
        if "refresh_token" not in credential_data or not credential_data["refresh_token"]:
            return JSONResponse(content={
                ...
                "success": False,
                "message": "该凭证没有refresh_token，无法刷新"
            }, status_code=400)

        log.info(f"[刷新Token] 开始刷新凭证: {filename_only}")

        # 调用credential_manager的刷新方法
        refreshed_data = await credential_manager._refresh_token(credential_data, filename_only)

        if refreshed_data:
            log.info(f"[刷新Token] 成功刷新凭证: {filename_only}")
            return JSONResponse(content={
                "filename": filename_only,
                "success": True,
                "message": "Token刷新成功",
                "new_expiry": refreshed_data.get("expiry", "未知")
            })
        else:
            log.warning(f"[刷新Token] 刷新失败: {filename_only}")
            return JSONResponse(content={
                "filename": filename_only,
                "success": False,
                "message": "Token刷新失败，可能refresh_token已过期"
            }, status_code=400)
...
@router.post("/creds/test-availability/{filename}")
async def test_single_credential_availability(
    filename: str,
    request: Request,
    ...
    try:
        ...
        log.info(f"[测试可用性] 开始测试凭证: {filename_only}, 使用模型: {test_model}")
        ...
        temp_manager = SingleCredentialManager(filename_only, credential_data)
        ...
        try:
            ...
            try:
                response_data = json.loads(response.body.decode('utf-8'))
                has_content = bool(response_data.get("candidates"))
            except:
                has_content = False

            # 判断可用性
            if status_code == 200 and has_content:
                log.info(f"[测试可用性] 凭证可用: {filename_only}")

                # 自动启用可用的账号并清除错误码
                try:
                    log.info(f"[测试可用性] 准备启用账号: {filename_only}")
                    await credential_manager.set_cred_disabled(filename_only, False)
                    log.info(f"[测试可用性] set_cred_disabled 调用完成")

                    # 记录成功调用，这会自动清除错误码
                    await credential_manager.record_api_call_result(filename_only, True)
                    log.info(f"[测试可用性] record_api_call_result 调用完成")

                    log.info(f"[测试可用性] 已自动启用账号并清除错误码: {filename_only}")
                    action_taken = "已自动启用并清除错误码"
                except Exception as enable_error:
                    log.error(f"[测试可用性] 自动启用失败: {filename_only}, 错误: {enable_error}", exc_info=True)
                    action_taken = f"启用失败: {str(enable_error)}"

                return JSONResponse(content={
                    "filename": filename_only,
                    "available": True,
                    "status_code": status_code,
                    "message": f"账号可用，{action_taken}"
                })
            else:
                log.warning(f"[测试可用性] 凭证不可用: {filename_only}, 状态码: {status_code}")

                # 自动禁用不可用的账号
                try:
                    await credential_manager.set_cred_disabled(filename_only, True)
                    log.info(f"[测试可用性] 已自动禁用账号: {filename_only}")
                    action_taken = "已自动禁用"
                except Exception as disable_error:
                    log.warning(f"[测试可用性] 自动禁用失败: {filename_only}, 错误: {disable_error}")
                    action_taken = "禁用失败"
                ...
                return JSONResponse(content={
                    ...
                    "available": False,
                    "status_code": status_code,
                    "message": f"账号不可用 (状态码: {status_code})，{action_taken}"
                })

        except Exception as req_error:
            log.error(f"[测试可用性] 请求失败: {filename_only}, 错误: {req_error}")
            return JSONResponse(content={
                "filename": filename_only,
                "available": False,
                "message": f"测试请求失败: {str(req_error)}"
            }, status_code=500)

    except HTTPException:
        raise
    except Exception as e:
        log.error(f"测试可用性失败: {e}")
        raise HTTPException(status_code=500, detail=str(e))


@router.post("/creds/batch-refresh-tokens")
async def batch_refresh_tokens(
    request: CredFileBatchActionRequest,
    token: str = Depends(verify_token)
):
    """
    批量刷新多个凭证的access token
    """
    try:
        await ensure_credential_manager_initialized()
        ...
        filenames = request.filenames
        if not filenames:
            raise HTTPException(status_code=400, detail="未提供文件名列表")

        log.info(f"[批量刷新Token] 开始刷新 {len(filenames)} 个凭证")
            ...
            try:
                ...
                if "refresh_token" not in credential_data or not credential_data["refresh_token"]:
                    results.append({
                        "filename": filename,
                        "success": False,
                        "message": "没有refresh_token"
                    })
                    failed_count += 1
                    continue

                # 刷新token
                refreshed_data = await credential_manager._refresh_token(credential_data, filename)
        ...
        log.error(f"批量刷新Token失败: {e}")
        raise HTTPException(status_code=500, detail=str(e))


@router.post("/creds/batch-test-availability")
async def batch_test_availability(
    request: CredFileBatchActionRequest,
    token: str = Depends(verify_token)
):
    """
    ...
    try:
        ...
        log.info(f"[批量测试可用性] 开始测试 {len(filenames)} 个凭证")
        ...
        # 并发测试（限制并发数）
        async def test_single(filename):
            nonlocal available_count, unavailable_count
        ...
        for i in range(0, len(filenames), batch_size):
            ...
            batch_results = await asyncio.gather(*[test_single(f) for f in batch])
            results.extend(batch_results)

        message = f"批量测试完成：可用 {available_count} 个，不可用 {unavailable_count} 个"
        log.info(f"[批量测试可用性] {message}")
        ...
        log.error(f"批量测试可用性失败: {e}")
        raise HTTPException(status_code=500, detail=str(e))


@router.post("/creds/refresh-all-tokens")
async def refresh_all_tokens(token: str = Depends(verify_token)):
    """
    刷新所有凭证的access token
    """
    try:
        ...
        log.info(f"[刷新全部Token] 开始刷新 {len(all_credentials)} 个凭证")
        ...
        message = f"刷新全部Token完成：成功 {success_count} 个，失败 {failed_count} 个"
        log.info(f"[刷新全部Token] {message}")

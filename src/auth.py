        ...
        try:
            ...
            if flow_data.get('get_all_projects', False) or get_all_projects:
                ...
                if user_projects:
                            ...
                            try:
                                ...
                                if '429' in error_str or 'rate limit' in error_str.lower() or 'quota' in error_str.lower():
                                    if attempt < max_retries - 1:
                                        ...
                                        log.warning(f"项目 {project_name} 触发速率限制，等待 {wait_time:.1f} 秒后重试 (尝试 {attempt + 1}/{max_retries})...")
                                        await asyncio.sleep(wait_time)
                                        continue
                                    else:
                                        log.error(f"项目 {project_name} 达到最大重试次数，仍然失败: {e}")
                                else:
                    ...
                    # 分批并发处理所有项目（避免同时发起过多请求）
                    batch_size = 10  # 每批处理10个项目
                    request_interval = 0.5  # 每个请求之间间隔0.5秒

                    log.info(f"开始分批处理 {len(user_projects)} 个项目（每批 {batch_size} 个，请求间隔 {request_interval} 秒）...")

                    all_results = []
                    for i in range(0, len(user_projects), batch_size):
                        batch = user_projects[i:i + batch_size]
                        batch_num = (i // batch_size) + 1
                        total_batches = (len(user_projects) + batch_size - 1) // batch_size

                        log.info(f"处理第 {batch_num}/{total_batches} 批，包含 {len(batch)} 个项目...")

                        # 为批次中的每个项目添加延迟
                        async def process_with_delay(project_info, delay):
                            await asyncio.sleep(delay)
                            return await process_single_project(project_info)

                        # 创建带延迟的任务
                        tasks = [
                            process_with_delay(project_info, idx * request_interval)
                            for idx, project_info in enumerate(batch)
                        ]
                        ...
                        # 批次之间额外等待
                        if i + batch_size < len(user_projects):
                            log.info(f"批次 {batch_num} 完成，等待 1 秒后处理下一批...")
...
TOKEN_EXPIRY = 31536000  # 1年令牌过期时间 (365天 * 24小时 * 3600秒)

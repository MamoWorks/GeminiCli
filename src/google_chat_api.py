            ...
            if is_streaming:
                ...
                try:
                    ...
                    if resp.status_code == 429:
                        ...
                        # 显示详细的429错误信息
                        if response_content:
                            log.error(f"[{current_file}] Google API returned status 429 (STREAMING). Response details: {response_content[:500]}")
                        else:
                            log.error(f"[{current_file}] Google API returned status 429 (STREAMING) - quota exhausted, no response details available")
                        ...
                        # 显示详细的错误信息
                        if response_content:
                            log.error(f"[{current_file}] Google API returned status {resp.status_code} (STREAMING). Response details: {response_content[:500]}")
                        else:
                            log.error(f"[{current_file}] Google API returned status {resp.status_code} (STREAMING) - no response details available")
        ...
        async def cleanup_and_error():
            ...
            if resp.status_code == 429:
                if response_content:
                    ...
                    log.error(f"[{current_file}] Google API returned status 429 (STREAMING)")
            else:
                if response_content:
                    log.error(f"[{current_file}] Google API returned status {resp.status_code} (STREAMING). Response details: {response_content[:500]}")
                else:
                    log.error(f"[{current_file}] Google API returned status {resp.status_code} (STREAMING)")
...
async def _handle_non_streaming_response(resp, credential_manager: CredentialManager = None, model_name: str = "", current_file: str = None) -> Response:
    ...
    if resp.status_code == 200:
        ...
        if resp.status_code == 429:
            if response_content:
                log.error(f"[{current_file}] Google API returned status 429 (NON-STREAMING). Response details: {response_content[:500]}")
            else:
                log.error(f"[{current_file}] Google API returned status 429 (NON-STREAMING)")
        else:
            if response_content:
                log.error(f"[{current_file}] Google API returned status {resp.status_code} (NON-STREAMING). Response details: {response_content[:500]}")
            else:
                log.error(f"[{current_file}] Google API returned status {resp.status_code} (NON-STREAMING)")

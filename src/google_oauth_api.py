                ...
                try:
                    ...
                    if check_response.status_code == 429:
                        if attempt < max_retries - 1:
                            ...
                            log.warning(f"检查服务 {service} 时遇到速率限制，等待 {wait_time:.1f} 秒后重试...")
                            await asyncio.sleep(wait_time)
                            continue
                        else:
                            log.error(f"检查服务 {service} 达到最大重试次数")
                            raise Exception(f"Rate limit exceeded for service check: {service}")
                    ...
                    for enable_attempt in range(max_retries):
                        try:
                            ...
                            # 处理429错误
                            if enable_response.status_code == 429:
                                if enable_attempt < max_retries - 1:
                                    wait_time = retry_delay * (2 ** enable_attempt)
                                    log.warning(f"启用服务 {service} 时遇到速率限制，等待 {wait_time:.1f} 秒后重试...")
                                    await asyncio.sleep(wait_time)
                                    continue
                                else:
                                    log.error(f"启用服务 {service} 达到最大重试次数")
                                    raise Exception(f"Rate limit exceeded for service enable: {service}")
                            ...
                            if enable_response.status_code in [200, 201]:
                                ...
                                if "already enabled" in error_data.get("error", {}).get("message", "").lower():
                                    ...
                                    log.warning(f"启用服务 {service} 时出现警告: {error_data}")
                                    break
                            else:
                                log.warning(f"启用服务 {service} 失败: {enable_response.status_code} - {enable_response.text}")
                                break

                        except Exception as e:
                            if enable_attempt < max_retries - 1:
                                log.warning(f"启用服务 {service} 时发生异常，重试中: {e}")
                                await asyncio.sleep(retry_delay * (2 ** enable_attempt))
                                continue
                            else:
                                log.warning(f"启用服务 {service} 时发生异常: {e}")
                                raise
                    ...
                    if attempt < max_retries - 1:
                        ...
                        try:
                            enable_response = await post_async(enable_url, headers=headers, json={})
                            if enable_response.status_code in [200, 201, 400]:
                                log.info(f"尝试启用服务: {service}")
                        except:
                            pass

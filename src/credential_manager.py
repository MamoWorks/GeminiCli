...
class CredentialManager:
    ...
    async def _load_current_credential(self) -> Optional[Tuple[str, Dict[str, Any]]]:
        ...
        try:
            ...
            if should_refresh:
                # 检查是否有refresh_token可以刷新
                if "refresh_token" not in credential_data or not credential_data["refresh_token"]:
                    log.warning(f"Token已过期但没有refresh_token，无法刷新: {current_file}")
                    return None
                ...
                if refreshed_data:
                    ...
                    log.info(f"Token刷新成功: {current_file}")
                else:
                    log.error(f"Token刷新失败: {current_file}")
                    return None
            else:
                log.debug(f"Token仍然有效，无需刷新: {current_file}")
        ...
        async with self._operation_lock:
            ...
            for attempt in range(max_attempts):
                try:
                    ...
                    if result:
                        current_file, credential_data = result
                        # 打印当前使用的凭证名
                        log.info(f"使用凭证: {current_file} (索引: {self._current_credential_index})")

                        # 成功获取凭证后，轮换到下一个（为下次请求准备）
    ...
    async def _rotate_credential(self):
        ...
        log.debug(f"Rotated to credential index {self._current_credential_index}")

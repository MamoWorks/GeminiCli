...
# Windows系统下抑制ProactorEventLoop的ConnectionResetError异常
if sys.platform == 'win32':
    def silence_event_loop_closed(func):
        """装饰器：抑制Windows下的事件循环关闭异常"""
        def wrapper(self, *args, **kwargs):
            try:
                return func(self, *args, **kwargs)
            except (RuntimeError, ConnectionResetError) as e:
                if 'Event loop is closed' in str(e) or 'WinError 10054' in str(e):
                    pass  # 忽略这些无害的异常
                else:
                    raise
        return wrapper

    # 修补asyncio的ProactorEventLoop
...
# 静态文件服务 - 提供client目录下的静态资源（CSS、JS等）
